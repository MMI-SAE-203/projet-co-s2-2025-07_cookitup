import { c as createComponent, r as renderComponent, b as renderScript, a as renderTemplate, f as defineScriptVars, e as addAttribute, m as maybeRenderHead } from '../chunks/astro/server_CmPnL_Nc.mjs';
import 'kleur/colors';
import { $ as $$Layout } from '../chunks/Layout_BgUZg3SC.mjs';
import { $ as $$Hero } from '../chunks/Hero_sNwyMDeS.mjs';
import { b as getAllPartenaires } from '../chunks/backend_C2hqppH8.mjs';
...
const $$Partenaires = createComponent(async ($$result, $$props, $$slots) => {
  ...
  function getImageUrl(partenaire, field) {
    ...
    return `http://127.0.0.1:8090/api/files/partenaires/${partenaire.id}/${partenaire[field]}`;
  }
...
</button> </div> ` }), [...new Set(partenaires.map((p) => p.ville))].map((ville) => renderTemplate`<option${addAttribute(ville, "value")}>${ville}</option>`), totalPartenaires, partenaires.map((partenaire) => renderTemplate`<div class="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow partenaire-card"${addAttribute(Array.isArray(partenaire.categories) ? partenaire.categories.join(",") : "", "data-categories")}${addAttribute(parseInt(partenaire.reduction?.replace("%", "") || "0"), "data-reduction")}${addAttribute(partenaire.id, "data-id")}${addAttribute(partenaire.latitude, "data-lat")}${addAttribute(partenaire.longitude, "data-lng")}> <!-- Image du magasin --> <div class="relative h-48"> <img${addAttribute(getImageUrl(partenaire, "image") || "/placeholder.svg", "src")}${addAttribute(`Magasin ${partenaire.nom}`, "alt")} class="w-full h-full object-cover"> <!-- Badge réduction --> ${partenaire.reduction && renderTemplate`<div class="absolute top-4 right-4 bg-red-500 text-white px-3 py-1 rounded-full font-bold">
-${partenaire.reduction} </div>`} <!-- Badge Cookup Pass actif --> <div class="absolute top-4 left-4 bg-gradient-to-r from-green-400 to-emerald-500 text-white px-2 py-1 rounded-full text-xs font-bold">
✅ ACTIF
</div> </div> <div class="p-6"> <!-- Logo et nom --> <div class="flex items-center mb-4"> <img${addAttribute(getImageUrl(partenaire, "logo") || "/placeholder.svg", "src")}${addAttribute(`Logo ${partenaire.nom}`, "alt")} class="h-12 w-auto mr-4"> <div> <h3 class="text-xl font-bold">${partenaire.nom}</h3> <p class="text-sm text-gray-500">${partenaire.ville}</p> </div> </div> <!-- Description --> <p class="text-gray-600 mb-4 text-sm leading-relaxed"> ${partenaire.description} </p> <!-- Offre débloquée --> ${partenaire.reduction && partenaire.code_promo && renderTemplate`<div class="bg-green-50 border border-green-200 rounded-lg p-4 mb-4"> <div class="flex items-center justify-between mb-2"> <span class="text-2xl font-bold text-green-600"> ${partenaire.reduction} </span> ${partenaire.validite_offre && renderTemplate`<span class="text-xs text-gray-500">
Valide jusqu'au ${partenaire.validite_offre} </span>`} </div> <div class="bg-white border border-dashed border-green-400 rounded p-2 text-center"> <span class="text-xs text-gray-500">Code promo :</span> <span class="font-mono font-bold text-green-600 ml-1 promo-code"> ${partenaire.code_promo} </span> </div> </div>`} <!-- Catégories --> ${Array.isArray(partenaire.categories) && partenaire.categories.length > 0 && renderTemplate`<div class="flex flex-wrap gap-1 mb-4"> ${partenaire.categories.map((categorie) => renderTemplate`<span class="bg-gray-100 text-gray-700 px-2 py-1 rounded text-xs"> ${categorie} </span>`)} </div>`} <!-- Informations pratiques --> <div class="space-y-2 text-sm text-gray-600 mb-4"> <div class="flex items-start"> <span class="mr-2">📍</span> <span>${partenaire.adresse}</span> </div> ${partenaire.telephone && renderTemplate`<div class="flex items-center"> <span class="mr-2">📞</span> <span>${partenaire.telephone}</span> </div>`} ${partenaire.horaires && renderTemplate`<div class="flex items-start"> <span class="mr-2">🕒</span> <span>${partenaire.horaires}</span> </div>`} </div> <!-- Actions --> <div class="flex gap-2"> ${partenaire.code_promo && renderTemplate`<button class="flex-1 bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded transition"${addAttribute(`copyPromoCode('${partenaire.code_promo}')`, "onclick")}>
📋 Copier le code
